...
DOCUMENTS_ROOT = BASE_DIR / "uploads" / "documents"
...
class DocumentService:
    ...
    def resolve_file_path(file_path: str) -> Path:
        """Return an absolute ``Path`` for a stored document."""

        path = Path(file_path)
        if not path.is_absolute():
            # Stored paths are relative to the backend directory so make
            # sure we resolve them the same way everywhere.
    ...
    def delete_document(
        ...
        try:
            ...
            try:
                if file_path:
                    resolved_path = DocumentService.resolve_file_path(file_path)
                    if resolved_path.exists():
                        resolved_path.unlink()
                        logger.info(f"Successfully deleted file: {resolved_path}")
                    else:
                        logger.warning(f"File not found on disk: {resolved_path}")
